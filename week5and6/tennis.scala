...
object tennisExercise {
	...
	val probP1Serve = 0.58
	val probP2Serve = 0.55
	...
	def mkSet(p1Games: Int, p2Games: Int, setNum: Int, server: Boolean): Element[Boolean] = {
		if ((setNum != 3) && (p1Games == 6) && (p2Games == 6)) tiebreak(0, 0, setNum, server)
	...
	def mkGame(p1Points: Int, p2Points: Int, server: Boolean): Element[Boolean] = {
		if ((p1Points >= 4) && ((p1Points - p2Points) >= 2)) Constant(true)
		else if ((p2Points >= 4) && ((p2Points - p1Points) >= 2)) Constant(false)
		else for {
