...
object BaseballSimulator {
	...
	def playGame(currBatterOrder: Int): Element[(scorecard, scorecard, Boolean)] = {
		...
		val initState2 = scorecard(batters2, pitcher1, 0, 1, "111", 1, 2, 3, currBatterOrder)
	...
	def playHalfInning(currState: scorecard): Element[scorecard] = {
		...
		for {
			...
			newBases <- if (play == "1B")
					    	...
					    	Select(0.4 -> "110", 0.1 -> "101", 0.5 -> "111")
		} yield {
			...
			if (play == "1B") {
				...
				if (newBases == "110") {
					...
					val newCurrBatter = batters(currBatterOrder).copy(t1B = new1B, tRBI = newRBI)
					...
					val newBatters = batters.updated(currBatterOrder, newCurrBatter)
					val newNewBatters = newBatters.updated(base3, newRunner3rd)
					currState.copy(bases = newBases, runs = newRuns, base1 = newBase1, base2 = newBase2, base3 = newBase3, batters = newNewBatters, currBatterOrder = newBatterOrder)
				}
			} else if (play == "2B") {
				...
				if (newBases == "010") {
					...
					val newCurrBatter = batters(currBatterOrder).copy(t2B = new2B, tRBI = newRBI)
					val newBatters = batters.updated(currBatterOrder, newCurrBatter)
					val newNewBatters = newBatters.updated(base3, newRunner3rd)
					val newNewNewBatters = newNewBatters.updated(base2, newRunner2nd)
				...
				val newRBI = batters(currBatterOrder).tRBI + 3
				val newCurrBatter = batters(currBatterOrder).copy(t3B = new3B, tRBI = newRBI)
				val newBatters = batters.updated(currBatterOrder, newCurrBatter)
				val newNewBatters = newBatters.updated(base3, newRunner3rd)
				val newNewNewBatters = newNewBatters.updated(base2, newRunner2nd)
				val newNewNewNewBatters = newNewNewBatters.updated(base1, newRunner1st)
				...
				val newCurrBatter = batters(currBatterOrder).copy(tHR = newHR, tRBI = newRBI, tR = newR)
				val newBatters = batters.updated(currBatterOrder, newCurrBatter)
				val newNewBatters = newBatters.updated(base3, newRunner3rd)
				val newNewNewBatters = newNewBatters.updated(base2, newRunner2nd)
				val newNewNewNewBatters = newNewNewBatters.updated(base1, newRunner1st)
				...
				val newRBI = batters(currBatterOrder).tRBI + 1
				val newCurrBatter = batters(currBatterOrder).copy(tTW = newTW, tRBI = newRBI)
				val newBatters = batters.updated(currBatterOrder, newCurrBatter)
				val newNewBatters = newBatters.updated(base3, newRunner3rd)
				currState.copy(bases = newBases, runs = newRuns, base1 = newBase1, base2 = newBase2, base3 = newBase3, batters = newNewBatters, currBatterOrder = newBatterOrder)
			} else if (play == "SO") {
				...
				val newCurrBatter = batters(currBatterOrder).copy(tSO = newSO)
				...
				if (newBases == "110") {
					...
					val newRunner3rd = batters(base3).copy(tR = newRun3rd)
					val newRBI = batters(currBatterOrder).tRBI + 1
					val newCurrBatter = batters(currBatterOrder).copy(tBO = newBO, tRBI = newRBI)
					val newBatters = batters.updated(currBatterOrder, newCurrBatter)
					val newNewBatters = newBatters.updated(base3, newRunner3rd)
	...
	def main(args: Array[String]) {
		...
		yield {
			val (result1, result2, result3) = result
			printBoxScore(result1, result2)
		}

		val team2Wins = triple._3 
		val algorithm = Importance(100, team2Wins)
		algorithm.start()
		println("Win Probability of team 2, when the game is tied, and bases "
				+ "are loaded with 1 out : " + algorithm.probability(team2Wins, true))
		algorithm.kill()
	}
}
